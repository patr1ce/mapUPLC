...
var Event = (function($) { return function(properties) {
      ...
      this.render = function (distance, zipcode) {
        ...
        if(!that.properties.is_campaign_office) {
            rendered.append($("<h5 class='time-info'/>").html((distance ? ("<span class='time-info-dist'>" + distance + "mi &nbsp;&nbsp;</span>") : "") + datetime + (endtime ? " - " + endtime : "" )));
        }
            
            //rendered.append(that.properties.is_official ? $("<h5 class='official-tag'/>").text("yoyoyo Official Event") : "")
        rendered.append($("<span/>").addClass("label-icon"))
...
var MapManager = (function($, d3, leaflet) {
  return (
    function(eventData, campaignOffices, zipcodes, options) {
      ...
      leaflet.mapbox.accessToken = "pk.eyJ1IjoidW5pdGVkNGVhcnRoLWllZXQiLCJhIjoiY2pub3BpdDA1MXlkMzNrbDc1Zmc2NWFhdyJ9.roCQIcV0V3Htgt7tlH_ciQ";
      ...
      var defaultCoord = options&&options.defaultCoord ? options.defaultCoord : {center: [25, 2.35], zoom: 2};
    ...
    */
    ...
    module.filter = function(zipcode, distance, sort, filterTypes) {
      ...
      if (targetZipcode == undefined || !targetZipcode) {
        $("#event-list").append("<li class='error lato'>"+locale["MAPMANAGER_UNKOWNZIP"]+" : " + zipcode ); //+ "&zip_radius%5b1%5d=100&country=US&radius_unit=mi\">Try our events page</a></li>");
        return;
      }
